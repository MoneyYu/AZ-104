...
#   私人 DNS 區域：lab04b.local
#   VNet-A（已連結、啟用自動註冊）內含 vm-a1、vm-a2
#   VNet-B（未連結）內含 vm-b1
#   手動 A 記錄：app.lab04b.local -> 10.40.1.100
#
# 教學重點：
...
#   nslookup vm-a2.lab04b.local      # 解析成功 -> 10.40.1.x
#   nslookup app.lab04b.local        # 解析成功 -> 10.40.1.100

# --- 3. 在 vm-b1 上（RDP 進入後於命令列執行）---
#   nslookup vm-a1.lab04b.local      # 失敗（NXDOMAIN）：VNet-B 尚未連結到區域
#   nslookup app.lab04b.local        # 失敗（NXDOMAIN）
...
# --- 5. 回到 vm-b1 重新測試 ---
#   nslookup vm-a1.lab04b.local      # 現在解析成功
#   nslookup app.lab04b.local        # 現在解析成功

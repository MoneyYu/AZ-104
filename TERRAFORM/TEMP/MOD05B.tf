...
resource "azurerm_virtual_machine_extension" "lab05bama" {
...
resource "azurerm_monitor_data_collection_rule_association" "lab05b" {
  name                    = "lab05b-dcra"
  target_resource_id      = azurerm_windows_virtual_machine.lab05b.id
  data_collection_rule_id = azurerm_monitor_data_collection_rule.vminsights.id
  description             = "VM Insights DCR association for lab05b"
}

resource "azurerm_monitor_data_collection_rule_association" "lab05b_otel" {
  name                    = "lab05b-otel-dcra"
  target_resource_id      = azurerm_windows_virtual_machine.lab05b.id
  data_collection_rule_id = azapi_resource.vminsights_otel.id
  description             = "OpenTelemetry metrics DCR association for lab05b"
}

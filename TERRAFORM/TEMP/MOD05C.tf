...
resource "azurerm_public_ip" "lab05c" {
  ...
  sku                 = "Standard"
  domain_name_label   = "${local.lab05c_name}-pip-${local.random_str}"
  tags                = local.default_tags
...
resource "azurerm_monitor_data_collection_rule_association" "lab05c_otel" {
  name                    = "lab05c-otel-dcra"
  target_resource_id      = azurerm_windows_virtual_machine.lab05c.id
  data_collection_rule_id = azapi_resource.vminsights_otel.id
  description             = "OpenTelemetry metrics DCR association for lab05c"
}

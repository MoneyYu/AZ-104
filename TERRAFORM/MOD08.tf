...
resource "azurerm_monitor_data_collection_rule_association" "lab08" {
  name                    = "lab08-dcra"
  target_resource_id      = azurerm_windows_virtual_machine.lab08.id
  data_collection_rule_id = azurerm_monitor_data_collection_rule.vminsights.id
  description             = "VM Insights DCR association for lab08"
}

resource "azurerm_monitor_data_collection_rule_association" "lab08_otel" {
  name                    = "lab08-otel-dcra"
  target_resource_id      = azurerm_windows_virtual_machine.lab08.id
  data_collection_rule_id = azapi_resource.vminsights_otel.id
  description             = "OpenTelemetry metrics DCR association for lab08"
}
